...
export default function Jogos() {
  ...
  const isJogoAoVivo = (jogo: any) => {
    if (jogo.resultado) return false;
    ...
    const dataFimJogo = new Date(dataJogo.getTime() + 2 * 60 * 60 * 1000);
  ...
  const jogosFiltrados = useMemo(() => {
    let jogosParaExibir = jogos.filter((j) => !j.resultado);
    ...
    <LayoutPage 
      ...
      extraHeader={
        <div className="flex gap-3">
          ...
          >
            <motion.span 
              animate={{ scale: filtro === "aoVivo" ? [1, 1.2, 1] : 1 }}
              transition={{ repeat: filtro === "aoVivo" ? Infinity : 0, duration: 1.5 }}
              className={`w-2 h-2 rounded-full ${filtro === "aoVivo" ? "bg-white" : "bg-red-600"}`}
            />
            Ao Vivo
            <AnimatePresence>
              {jogosAoVivoCount > 0 && (
                  ...
                  >
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2 text-slate-600">
                        ...
                        <span>
                          Transmissão: <span className="font-semibold text-slate-800">{jogo.transmissao || "TPA 1"}</span>
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-slate-600">
                        ...
                        <span>
                          Previsão: <span className="font-semibold text-slate-800">{jogo.clima || "Ensolarado"}</span>
                        </span>
                      </div>
